...
final class ServerController: ObservableObject {
    ...
    func start() {
        errorMessage = nil
        AppLog.info("Starting local server on preferred port \(port)", source: "server")
        Task {
            ...
            if running {
                AppLog.info("Server running on port \(self.port)", source: "server")
            } else if let error {
    ...
    func stop() {
        AppLog.info("Stopping local server", source: "server")
        Task {
    ...
    func restart() {
        errorMessage = nil
        AppLog.info("Restarting local server on preferred port \(port)", source: "server")
        Task {
            await RelayClient.shared.disconnect()
            await LocalHTTPServer.shared.stop()
            await LocalHTTPServer.shared.setPort(port)
            ...
            if running {
                ...
                AppLog.error("Server restart failed: \(error)", source: "server")
            }
    ...
    func setRelayEnabled(_ enabled: Bool) {
        ...
        AppLog.info("Relay \(enabled ? "enabled" : "disabled")", source: "relay")
        Task {
